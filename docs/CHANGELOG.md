...
> `v1.3`), без отдельных «v4»/«v5».

---

## v1.3 — критический фикс сборки + чистка предупреждений

Исправляет две ошибки, из-за которых мог не компилироваться на некоторых платформах через `nim check`/`nim c`, плюс несколько мелких находок, всплывших при полной проверке компилятором (`nim check` на Nim 1.6.14).
