...
Приложение разработано на языке программирования **Nim**. Все необходимые для интерполяции кадров модули FFmpeg линкуются статически.  
...
Эта версия исправляет две ошибки, из-за которых проект **вообще не
компилировался** через `nim check`/`nim c`, плюс несколько мелких находок,
всплывших при полной проверке компилятором (`nim check` на Nim 1.6.14).

**Critical (ошибки компиляции)**
