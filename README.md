...
Параллельно обрабатывая фрагменты видеофайла, приложение повышает FPS видео с помощью мощного фильтра `minterpolate`, кодирует в **x264**; аудио и субтитры копируются без изменений.

Приложение разработано на языке программирования **Nim**; статически линкует необходимые для интерполяции кадров модули FFmpeg.  
